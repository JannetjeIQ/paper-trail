...
**Type:** process
**State:** live
**Lives at:** `<scripts>/build_db.py`
**Source of truth:** itself. When it and the data dictionary disagree, this file wins.

## What it is

The nightly rebuild. Pulls **seven** reports from the job system's reporting API
and writes [jobs.sqlite](jobs-sqlite.md) from scratch: customers, orders,
...
- **It drops data silently, by design.** Test-customer orders are excluded. Quote-only
  ...
  and why tens of thousands of quote lines are simply unavailable. Legacy rows with impossible
...
- [jobs.sqlite](jobs-sqlite.md) : rebuilt entirely by this script. Every table, every index.
- Every reporting script downstream : they read a store whose shape this file decides.

## Does not hit

- **The live job system.** Read-only. It pulls, it never writes back.
